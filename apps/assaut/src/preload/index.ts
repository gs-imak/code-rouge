...
// `contextIsolation: true` + this preload script is the only legitimate path
// for the renderer to call into main. The renderer never touches `ipcRenderer`
// directly. Each method validates the response with Zod so a buggy main can't
// poison renderer state with the wrong shape.
