...
//   Phase 2 — reconnect with same deviceId, teamId=null (simulating local
//             storage wipe), expect to receive a `restore` message
...
async function phase2() {
  console.log('[phase2] reconnect post-wipe, expect restore')
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(URL)
    const timer = setTimeout(() => {
      ws.close()
